...
<div class="span4">
    <div class="control-group">
        ...
        <div class="controls{{ $errors->has('cedula_re') ? ' has-error' : '' }}">
            @if(strlen(trim($representante->cedula_re)) <= 2)
                <select id="select01" name="nacionalidad" class="chzn-select" style="width: 50px">
                    <option value="N-">N</option>
                    <option value="E-">E</option>
                </select>

                {!! Form::text('cedula', null, ['class' => 'form-control', 'title' => 'Ingrese el número de cédula del representante.', 'placeholder' => '25607932', 'style' => 'width: 130px;']) !!}
            @else
                {!! Form::text('cedula_re', null, ['class' => 'form-control', 'id' => 'dni_cedula', 'placeholder' => '178455996-9', 'title' => 'Introduzca la cédula del representante', 'disabled' => 'disabled']) !!}
            @endif
        </div>
    </div>
    <div class="control-group">
